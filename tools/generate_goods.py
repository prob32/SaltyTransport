...
# Balance dials (mirrors ARCHITECTURE_V3.md)
CAP_DELTA = 0.10     # max local price delta from shaping (+/-10%)
INJ_K = round(CAP_DELTA / 0.75, 4)  # delta -> injection: x = sig*K*min(buy,sell)
EMA_NEW = 0.5        # weight of the fresh Phase B target
EMA_OLD = 0.5        # weight of last month's applied injection
RATE_CAP = 0.9       # max growth per cycle as fraction of old value
RATE_FLOOR = 5       # minimum allowed growth (lets new shaping ramp up)
...
def gen_phase_b():
    ...
    L += ["# Cache local price estimate, economy scale, production and order floor",
          "# for all goods. Scope: state. Run once per state on the first Phase B day.",
          "stl_cache_state_prices = {"]
    L += s_clear_map("stl_price", names(), 1)
    L += s_clear_map("stl_econ", names(), 1)
    L += s_clear_map("stl_prod", names(), 1)
    L += s_clear_map("stl_minbs", names(), 1)
    for (g, _disp, base, _tq, _cat, _en) in enabled_goods():
        ...
        L += s_write("stl_minbs", g, "local_var:stl_min_bs", 2)
        L += [
            "\t\tif = {",
    ...
    for (g, _disp, base, _tq, _cat, _en) in enabled_goods():
        ...
        L += [
            "\t# --- signal -> capped injection target (ARCHITECTURE_V3.md) ---",
            "\t# sig = (WAD - WAS) / (WAD + WAS); x = sig x INJ_K x basis",
            "\t# basis = min(buy, sell), floored at 10% of economy scale so",
            "\t# pure producers/consumers still receive a small nudge.",
            f"\tset_local_variable = {{ name = stl_tot value = {{ value = local_var:stl_acc_wad_{g} add = local_var:stl_acc_was_{g} }} }}",
            "\tset_local_variable = { name = stl_x value = 0 }",
            "\tif = {",
            "\t\tlimit = {",
            "\t\t\tlocal_var:stl_tot > 0.01",
            f"\t\t\t{s_has('stl_minbs', g)}",
            f"\t\t\t{s_has('stl_econ', g)}",
            "\t\t}",
            f"\t\tset_local_variable = {{ name = stl_sig value = {{ value = local_var:stl_acc_wad_{g} subtract = local_var:stl_acc_was_{g} divide = local_var:stl_tot }} }}",
            f"\t\tset_local_variable = {{ name = stl_basis value = {s_read('stl_minbs', g)} }}",
            f"\t\tset_local_variable = {{ name = stl_econ_floor value = {{ value = {s_read('stl_econ', g)} multiply = 0.1 }} }}",
            "\t\tif = {",
            "\t\t\tlimit = { local_var:stl_econ_floor > local_var:stl_basis }",
            "\t\t\tset_local_variable = { name = stl_basis value = local_var:stl_econ_floor }",
            "\t\t}",
            f"\t\tset_local_variable = {{ name = stl_x value = {{ value = local_var:stl_sig multiply = {INJ_K} multiply = local_var:stl_basis }} }}",
            "\t}",
            "\t# persist targets (sparse: only meaningful values keep a key)",
            "\t# x > 0: raise price (depot buys / export side); x < 0: lower (sells / import side)",
            "\tif = {",
            f"\t\tlimit = {{ local_var:stl_x > {MIN_TRADE} }}",
        ]
        L += s_write("stl_eff_export", g, "local_var:stl_x", 2)
        L += ["\t}",
              "\tif = {",
              f"\t\tlimit = {{ NOT = {{ local_var:stl_x > {MIN_TRADE} }} }}"]
        L += s_remove("stl_eff_export", g, 2)
        L += ["\t}",
              "\tif = {",
              f"\t\tlimit = {{ local_var:stl_x < -{MIN_TRADE} }}",
              "\t\tset_local_variable = { name = stl_x_abs value = { value = 0 subtract = local_var:stl_x } }"]
        L += s_write("stl_eff_import", g, "local_var:stl_x_abs", 2)
        L += ["\t}",
              "\tif = {",
              f"\t\tlimit = {{ NOT = {{ local_var:stl_x < -{MIN_TRADE} }} }}"]
...
def gen_phase_c():
    g = "$GOODS$"
    L = [AUTOGEN,
         "# PHASE C v3 (see ARCHITECTURE_V3.md)",
         "# Dispatcher batches goods across days 21-28; stl_phase_c_good applies",
         "# the capped price-shaping injections: EMA + rate cap + decay,",
         "# zero-sum conservation, transport flavor and GUI bookkeeping.",
         ""]
...
def gen_partner_sguis():
    L = [AUTOGEN,
         "# Per-good scripted GUIs for the trade window (literal names; the",
         "# window's generated rows reference them directly).",
         ""]
    for g in names():
    ...
    L += ["# Row visibility: IsValid drives whether a good's row shows in the window.",
          ""]
    for g in names():
        L += [
            f"stl_row_vis_{g} = {{",
            ...
            "\tis_valid = {",
            "\t\tOR = {",
            f"\t\t\t{s_has('stl_last_export', g)}",
            f"\t\t\t{s_has('stl_last_import', g)}",
            f"\t\t\t{s_has('stl_eff_export', g)}",
            f"\t\t\t{s_has('stl_eff_import', g)}",
            "\t\t}",
            "\t}",
            "\tai_is_valid = { always = no }",
            "}",
...
GOODS_ICONS = {
    # icon filename differs from the good key only for these
    "engines": "locomotives.dds",
    "manowars": "man_o_wars.dds",
}


def goods_icon(g):
    return "gfx/interface/icons/goods_icons/" + GOODS_ICONS.get(g, g + ".dds")


# ============================================================
# 8b. TRADE WINDOW (fully literal rows; no dynamic GUI hops)
# Dynamic flag->Goods datacontexts (GetGoods(Scope.GetFlagName)) resolved
# to a null Goods at runtime (empty keys, placeholder icons). Rows are now
# generated per good with literal icon paths, literal names and literal
# ScriptValue bindings; per-row visibility via scripted_gui IsValid.
# ============================================================

def gen_trade_window():
    W = 760
    ROW = 26
    COL_GOOD = 170
    COL_NUM = 78
    L = [AUTOGEN,
         "# Standalone movable Internal Trade window, registered via",
         "# gui/scripted_widgets/stl_widgets.txt. Opened by stl_sgui_open_window",
         "# (depot panel button sets var:stl_window_state on the player country).",
         "",
         f"@stl_win_w       = {W}",
         f"@stl_row_h       = {ROW}",
         f"@stl_col_good    = {COL_GOOD}",
         f"@stl_col_num     = {COL_NUM}",
         "",
         "window = {",
         ...
         "\t\t}",
         "",
         "\t\t# goods rows (one literal block per good; hidden when not trading)",
         "\t\tscrollbox = {",
         "\t\t\tlayoutpolicy_horizontal = expanding",
    ...
    for (g, disp, _b, _t, _c, _en) in enabled_goods():
        vis = f"[GetScriptedGui('stl_row_vis_{g}').IsValid(GuiScope.SetRoot(State.MakeScope).End)]"
        L += [
            "\t\t\t\t\twidget = {",
            f'\t\t\t\t\t\tvisible = "{vis}"',
            ...
            "\t\t\t\t\t\t\t\t}",
            "\t\t\t\t\t\t\t}",
            f"\t\t\t\t\t\t\ttextbox = {{ minimumsize = {{ @stl_col_num @stl_row_h }} align = right text = \"[State.MakeScope.ScriptValue('stl_gui_trade_{g}')|0=]\" }}",
            f"\t\t\t\t\t\t\ttextbox = {{ minimumsize = {{ @stl_col_num @stl_row_h }} align = right text = \"[State.MakeScope.ScriptValue('stl_gui_target_{g}')|0=]\" }}",
            f"\t\t\t\t\t\t\ttextbox = {{ minimumsize = {{ @stl_col_num @stl_row_h }} align = right text = \"[State.MakeScope.ScriptValue('stl_gui_access_{g}')|0]%\" }}",
            f"\t\t\t\t\t\t\ttextbox = {{ minimumsize = {{ @stl_col_num @stl_row_h }} align = right text = \"£[State.MakeScope.ScriptValue('stl_gui_local_price_{g}')|1]\" }}",
            f"\t\t\t\t\t\t\ttextbox = {{ minimumsize = {{ @stl_col_num @stl_row_h }} align = right text = \"£[State.MakeScope.ScriptValue('stl_gui_market_price_{g}')|1]\" }}",
            f"\t\t\t\t\t\t\ttextbox = {{ minimumsize = {{ @stl_col_num @stl_row_h }} align = right text = \"[State.MakeScope.ScriptValue('stl_gui_transport_{g}')|0]\" }}",
            "\t\t\t\t\t\t\tbutton = {",
            "\t\t\t\t\t\t\t\tsize = { 24 24 }",
            "\t\t\t\t\t\t\t\tusing = default_button",
            '\t\t\t\t\t\t\t\ttooltip = "stl_trade_window_btn_import_tt"',
            '\t\t\t\t\t\t\t\ttext = "I"',
            f"\t\t\t\t\t\t\t\tonclick = \"[GetScriptedGui('stl_show_import_{g}').Execute(GuiScope.SetRoot(State.MakeScope).End)]\"",
            "\t\t\t\t\t\t\t}",
            "\t\t\t\t\t\t\tbutton = {",
            "\t\t\t\t\t\t\t\tsize = { 24 24 }",
            "\t\t\t\t\t\t\t\tusing = default_button",
            ...
            '\t\t\t\t\t\t\t\ttext = "E"',
            f"\t\t\t\t\t\t\t\tonclick = \"[GetScriptedGui('stl_show_export_{g}').Execute(GuiScope.SetRoot(State.MakeScope).End)]\"",
            "\t\t\t\t\t\t\t}",
            "\t\t\t\t\t\t}",
            "\t\t\t\t\t}",
        ]
    L += ["\t\t\t\t}",
          ...
          "\t\t\tvisible = \"[Not(GetScriptedGui('stl_sgui_has_trade').IsValid(GuiScope.SetRoot(State.MakeScope).End))]\"",
          "\t\t}",
          "",
          "\t\t# partner breakdown (filled by stl_show_import_/export_<good>)",
          "\t\tvbox = {",
          "\t\t\tlayoutpolicy_horizontal = expanding",
          ...
          '\t\t\t\t\t\t\t\t\t\ttext = "[State.GetName]"',
          "\t\t\t\t\t\t\t\t\t}",
          "\t\t\t\t\t\t\t\t\ttextbox = { minimumsize = { @stl_col_num @stl_row_h } align = right text = \"[State.MakeScope.ScriptValue('stl_sv_display_friction')|0]\" }",
          "\t\t\t\t\t\t\t\t\ttextbox = { minimumsize = { @stl_col_num @stl_row_h } align = right text = \"[State.MakeScope.ScriptValue('stl_sv_display_weight')|0]\" }",
          "\t\t\t\t\t\t\t\t\ttextbox = { minimumsize = { @stl_col_num @stl_row_h } align = right text = \"£[State.MakeScope.ScriptValue('stl_sv_display_source_price')|1]\" }",
          "\t\t\t\t\t\t\t\t\ttextbox = { minimumsize = { @stl_col_num @stl_row_h } align = right text = \"[State.MakeScope.ScriptValue('stl_sv_display_contribution')|0]%\" }",
          "\t\t\t\t\t\t\t\t}",
          "\t\t\t\t\t\t\t}",
          "\t\t\t\t\t\t}",
          "\t\t\t\t\t}",
          ...
          "}",
          ""]
    write_file("gui/stl_trade_window.gui", "\n".join(L))
